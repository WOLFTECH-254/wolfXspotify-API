# Disclaimer

**Project:** WOLF TRACK API · Silent Wolf Engine  
**Owner:** SILENT WOLF · WOLF TECH  
**GitHub:** [https://github.com/WOLFTECH-254/](https://github.com/WOLFTECH-254/)

---

## No Affiliation with Spotify

WOLF TRACK API is an **independent, unofficial project**. It is not affiliated with, endorsed by, sponsored by, or in any way connected to **Spotify AB**, its subsidiaries, or affiliates.

The project name "WOLF TRACK" is distinct from "Spotify" and is used solely as a project identifier for this open-source API gateway.

---

## Data Ownership

All music data, metadata, artwork, track information, artist profiles, and related content returned by this API is the property of **Spotify AB** and its licensors. WOLF TECH does not:

- Own any music or content data
- Store or cache Spotify content permanently
- Claim any rights over Spotify's intellectual property

---

## No Warranty

This software is provided **"as is"** without warranty of any kind. WOLF TECH makes no guarantees regarding:

- Uptime or availability of the service
- Accuracy or completeness of returned data
- Compatibility with future versions of Spotify's platform
- Uninterrupted or error-free operation

---

## Use at Your Own Risk

Use of this API is entirely at your own risk. WOLF TECH and SILENT WOLF shall not be held liable for any damages, losses, or legal issues arising from the use of this service, including but not limited to:

- Violations of Spotify's Terms of Service by end users
- Data inaccuracies or service outages
- Application failures caused by reliance on this API

---

## Compliance Responsibility

Users are solely responsible for ensuring their applications comply with:

- [Spotify's Terms of Service](https://www.spotify.com/legal/end-user-agreement/)
- [Spotify's Developer Policy](https://developer.spotify.com/terms)
- All applicable local laws and regulations

---

## Changes

This disclaimer may be updated at any time. The latest version will always be available in this repository and at the project's homepage.

---

## Contact

- **GitHub:** [https://github.com/WOLFTECH-254/](https://github.com/WOLFTECH-254/)
- **Maintainer:** SILENT WOLF · WOLF TECH
- **Location:** Kenya 🇰🇪

---

© 2026 WOLF TECH · SILENT WOLF — All rights reserved.